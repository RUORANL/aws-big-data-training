...
## 实验1-2 - 配置Kinesis Data Generator

点击下方的“启动堆栈”以启动Cloudformation堆栈创建，然后如下图所示点击“下一步”。  
{{< button href="https://console.aws.amazon.com/cloudformation/home?#/stacks/new?stackName=CognitoUserForKDG&templateURL=https://big-data-training-cfn-templates.s3.amazonaws.com/SetupCognitoForKDG.yaml" >}}启动堆栈{{< /button >}} 

## 模板介绍

**描述**：此模板创建一个Amazon Cognito用户池和身份池，并带有单个用户。它为身份池中的认证用户分配一个角色，以使用户能够使用Kinesis Data Generator工具。
...
- **Username（用户名）**：
  - **描述**：您希望在Amazon Cognito中创建的用户的用户名。
  - **类型**：字符串
  - **允许的模式**：`^(?=\\s*\\S).*$`
  - **约束描述**：不能为空

- **Password（密码）**：
  - **描述**：您希望在Amazon Cognito中创建的用户的密码。
  - **类型**：字符串
  - **允许的模式**：`^(?=.*[A-Za-z])(?=.*\\d)[A-Za-z\\d]{6,}$`
  - **约束描述**：至少需要6个字母数字字符，并至少包含一个数字

### 资源：

1. **KinesisDataGeneratorSecret**：
   - **类型**：`AWS::SecretsManager::Secret`
   - **属性**：为Kinesis Data Generator的Cognito用户创建的秘密

2. **StagingLambdaRole**：
   - **类型**：`AWS::IAM::Role`
   - **属性**：为Lambda函数创建的IAM角色
...
- **KinesisDataGeneratorUrl**：
  - **描述**：Kinesis Data Generator的URL。
  - **值**：[Kinesis Data Generator的URL](https://awslabs.github.io/amazon-kinesis-data-generator/web/producer.html?${SetupCognitoCustom.Querystring})

- **KinesisDataGeneratorCognitoUser**：
  - **描述**：我们在AWS Secrets中保存了您的Cognito用户/密码。
  - **值**：KinesisDataGeneratorSecret的引用
